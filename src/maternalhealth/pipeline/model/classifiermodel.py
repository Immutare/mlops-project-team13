...
class ClassifierModel(InformalModelInterface):
    ...
    def __init__(self, versionLabel: str, randomSeed=42, asTesting=False):
        ...
        self.BASE_DUMP_DIRECTORY = (
            f"{self.MODEL_DUMP_PATH}{'test/' if self.USE_AS_TEST else ''}"
        )

        # Definir los nombres de los archivos de los modelos
        self.knn_model_filename = (
            f"{self.BASE_DUMP_DIRECTORY}knn_model_{versionLabel}.pkl"
        )
        self.rf_model_filename = (
            f"{self.BASE_DUMP_DIRECTORY}rf_model_{versionLabel}.pkl"
        )
        self._label_encoder_filename = (
            f"{self.BASE_DUMP_DIRECTORY}label_encoder_{versionLabel}.pkl"
        )
        self.comparison_filename = (
            f"{self.BASE_DUMP_DIRECTORY}model_comparison_{versionLabel}.txt"
        )
    ...
    def cleanAndPreprocessData(self, X: DataFrame, y: DataFrame):
        ...
        # Se añade una pequeña constante para evitar división por 0
    ...
    def splitTrainTestVal(
        ...
        Xtrain, Xval, ytrain, yval = train_test_split(
            Xtrain,
            ytrain,
            test_size=0.1764705882352941,
            random_state=self.RANDOM_STATE_SEED,
        )  # 0.1764705882352941 x 0.85 = 0.15
        ...
        # # Peso de las clases de acuerdo al porcentaje de los datos
        # # encontrados en EDA
        ...
        # # Se usará el inverso de la proporción para definir los pesos de las
        # # clases.
    ...
    def getOutliers(self, X: DataFrame):
        ...
        # Identificar outliers
        outliers = (X < (Q1 - self.IQ_LOWER_RANGE * IQR)) | (
            X > (Q3 + self.IQ_LOWER_RANGE * IQR)
        )
    ...
    def normalizeData(X):
        ...
        Args:
            X (pd.DataFrame): El DataFrame con los datos a normalizar.
    ...
    def datasetSummary(self, X, Y):
        """
        Función que une X (features) e Y (target), realiza los cálculos de
        outliers, skewness y kurtosis para las variables numéricas, y genera
        una tabla consolidada con esta información.
        ...
        Retorna:
        final_summary_df -- DataFrame con la información de las columnas unidas
        de X y Y, incluyendo outliers, skewness y kurtosis para las variables
        numéricas.
        """
        ...
        outliers = (X[numeric_columns] < (Q1 - 1.5 * IQR)) | (
            X[numeric_columns] > (Q3 + 1.5 * IQR)
        )
        ...
        # 9. Crear un DataFrame con los cálculos
        # (outliers, skewness y kurtosis)
        ...
        final_summary_df = pd.concat(
            [info_df.set_index("Column"), summary_df], axis=1
        ).reset_index()

        # 11. Llenar NaN para las columnas no numéricas y target (Y) en los
        # cálculos de skewness, kurtosis y outliers
        final_summary_df.fillna(
            value={"Outliers": np.nan, "Skewness": np.nan, "Kurtosis": np.nan},
            inplace=True,
        )
    ...
    def datasetVisualization(
        self, X: DataFrame, Y: DataFrame, title="Dataset Overview"
    ):
        X.head()
        ...
        )  # `y` ajusta la posición vertical del título

        # Crear una cuadrícula con 3 filas y num_features columnas
        gs = gridspec.GridSpec(
            3, num_features, height_ratios=[1, 1, 2], width_ratios=[1] * 6
        )
        ...
        # Subgráfico para la matriz de correlación, ocupando dos columnas
        ...
        ax_corr.set_xticklabels(
            ax_corr.get_xticklabels(), rotation=45, horizontalalignment="right"
        )
        ...
        for p in ax_labels.patches:
            ax_labels.annotate(
                str(p.get_height()), (p.get_x() * 1.005, p.get_height() * 1.01)
            )
    ...
    def trainKNN(
        ...
        """
        Función para entrenar un modelo K-Nearest Neighbors (KNN) utilizando
        ...
        n_neighbors : list, opcional
            Lista de enteros que representan el número de vecinos a considerar
            en el algoritmo KNN.
            Valor por defecto: [5, 10, 20]

        weights : list, opcional
            Lista que especifica el tipo de ponderación que se aplicará a los
            vecinos.
            Opciones: ['uniform', 'distance']
            Valor por defecto: ['uniform', 'distance']
            - 'uniform': Todos los vecinos tienen la misma importancia.
            - 'distance': Los vecinos más cercanos tienen más peso que los más
            alejados.

        Distance : list, opcional
            Lista de enteros que definen la distancia que se utilizará en la
            métrica.
        ...
        best_knn_model : KNeighborsClassifier
            El mejor modelo KNN encontrado durante la búsqueda de
            hiperparámetros utilizando GridSearchCV.
        """
    ...
    def trainRF(
        ...
        """
        Función para entrenar un modelo Random Forest utilizando búsqueda de
        ...
        class_weight : dict
            Diccionario que define los pesos de las clases para compensar el
            desbalance de datos.
        ...
        max_depth : list, opcional
            Lista de enteros que especifica la profundidad máxima de los
            árboles.
            Valor por defecto: [3, 5, 10]

        min_samples_split : list, opcional
            Lista de enteros que define el número mínimo de muestras necesarias
            para dividir un nodo.
            Valor por defecto: [3, 5, 10, 20]

        min_samples_leaf : list, opcional
            Lista de enteros que especifica el número mínimo de muestras
            necesarias para estar en una hoja.
            Valor por defecto: [1, 2, 4]

        bootstrap : list, opcional
            Lista que especifica si se utilizará el muestreo con reemplazo
            (bootstrap).
        ...
        best_rf_model : RandomForestClassifier
            El mejor modelo Random Forest encontrado durante la búsqueda de
            hiperparámetros utilizando GridSearchCV.
        """
        ...
        param_grid = {
            ...
            "min_samples_split": min_samples_split,  # Número mínimo de muestras para dividir un nodo  # noqa: E501
            "min_samples_leaf": min_samples_leaf,  # Número mínimo de muestras en una hoja  # noqa: E501
        ...
        grid_search = GridSearchCV(
            estimator=random_forest_model,
            param_grid=param_grid,
            scoring="f1_weighted",  # Se pueden usar otras métricas
            cv=3,
            verbose=0,
            n_jobs=-1,
        )
    ...
    def trainAndSaveRFModel(self, X_train, Y_train):
        ...
        print(
            f"Modelo Random Forest guardado exitosamente como {self.rf_model_filename}."  # noqa: E501
        )
        return RF_Model
    ...
    def processAndTrainModels(
        ...
        # Guardar un archivo con la información de qué modelo es mejor y su
        # rendimiento
        ...
        print(
            f"Información de comparación guardada como {self.comparison_filename}."  # noqa: E501
        )

    def loadModel(self, version_label="", model_type=""):
        """
        Cargar el modelo entrenado basado en el version_label y el tipo de
        modelo especificado.

        Parámetros:
        -----------
        version_label : str
            El string que representa la versión de los modelos (ej:: "v1.2").
            Si no se pasa, se usara por defecto el pasado por el constructor

        model_type : str, opcional
            El tipo de modelo a cargar: "KNN" para K-Nearest Neighbors, "RF"
            para Random Forest, o si está vacío, carga el mejor modelo basado
            }en el archivo de comparación.
        ...
        if version_label:
            ...
            comparisonFilename = f"{self.BASE_DUMP_DIRECTORY}model_comparison_{version_label}.txt"  # noqa: E501
        else:
        ...
        # Si model_type está vacío, cargar el modelo mejor basado en el archivo
        # de comparación
        if model_type == "":
            try:
                ...
                raise ValueError(
                    f"No se encontró el archivo de comparación para la versión: {version_label}"  # noqa: E501
                )
        ...
        if model_type == "KNN":
            ...
            raise ValueError(
                f"Tipo de modelo no reconocido: {model_type}. Debe ser 'KNN', 'RF' o 'Random Forest'."  # noqa: E501
            )
    ...
    def evaluate_model_performance(self, y_true, y_pred, model_name="Model"):
        """
        Evalúa el rendimiento del modelo usando varias métricas de
        clasificación.
