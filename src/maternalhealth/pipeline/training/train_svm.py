...
def trainSVM(X, Y, **kwargs):
    ...
    --------
    best_svm_model : SVC
        El mejor modelo SVM encontrado durante la búsqueda de hiperparámetros
    ...
    grid_search = GridSearchCV(
        estimator=SVC(random_state=kwargs["seed"]),
...
def main():
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        with open(
            os.path.join(out_path, "mlflow_run_id_svm_model.txt"), "w"
        ) as f:
            f.write(run_id)
