...
def trainTree(X, Y, **kwargs):
    """
    Función para entrenar un modelo DecisionTreeClassifier utilizando búsqueda
    de hiperparámetros con GridSearchCV.
    ...
    best_decision_tree_model : DecisionTreeClassifier
        El mejor modelo DecisionTreeClassifier encontrado durante la búsqueda
        de hiperparámetros utilizando GridSearchCV.
    """
    ...
    grid_search = GridSearchCV(
        estimator=DecisionTreeClassifier(random_state=kwargs["seed"]),
...
def main():
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        # else:
        # Si no hay una búsqueda de hiperparámetros, guardar manualmente
        ...
        mlflow.sklearn.log_model(decision_tree_model, "DT_model")

        # Guardar el run_id para compartirlo con evaluate.py
        run_id = run.info.run_id
        with open(
            os.path.join(out_path, "mlflow_run_id_decision_tree_model.txt"),
            "w",
        ) as f:
            f.write(run_id)
