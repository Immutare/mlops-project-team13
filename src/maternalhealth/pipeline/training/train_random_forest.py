...
def train_random_forest(X, Y, **kwargs):
    """
    Función para entrenar un modelo RandomForestClassifier utilizando búsqueda
    de hiperparámetros
    con GridSearchCV.
    ...
    best_random_forest_model : RandomForestClassifier
        El mejor modelo RandomForestClassifier encontrado durante la búsqueda
...
def main():
    ...
    # Entrenar y guardar el modelo RandomForest usando los parámetros de
    # `params.yaml`
    with mlflow.start_run(run_name=run_name) as run:
        random_forest_model, best_params = train_random_forest(
        ...
        mlflow.sklearn.log_model(random_forest_model, "RF_model")

        # Guardar el run_id para compartirlo con evaluate.py
        run_id = run.info.run_id
        with open(
            os.path.join(out_path, "mlflow_run_id_random_forest_model.txt"),
            "w",
        ) as f:
            f.write(run_id)
