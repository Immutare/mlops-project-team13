...
def train_logistic(X, y, **kwargs):
    ...
    grid_search = GridSearchCV(estimator=LogisticRegression(random_state=kwargs["seed"]),
...
def main():
    if len(sys.argv) != 3:
        ...
        sys.stderr.write("\tpython train_logistic.py preprocessed-dir-path model-dir-path\n")
        sys.exit(1)
    ...
    run_name = f"Train Logistic Regression"
    
    # Entrenar y guardar el modelo Logistic Regression usando los parámetros de `params.yaml`
